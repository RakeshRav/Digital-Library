...
public class ModifyBookServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String target="AdminHome.jsp?status=Book Modification Failed";
        GetBooksBean bean;
        Vector v;
        ...
        try
		{
		    ...
		    if(i>0)
				target="AdminHome.jsp?status=Book Modification Success";
			else
				target="AdminHome.jsp?status=Book Modification Failed";
		}
        catch(Exception e)
		{
        	e.printStackTrace();
